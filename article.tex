...
% By conducting a thorough literature review, this study aimed to build a solid foundation of existing knowledge, identify gaps, and position the findings from the Bayesian Network models within the broader context of sustainability research in viticulture. This approach ensured that the study's results were robust and scientifically sound and practically relevant and actionable for industry stakeholders.

\subsection*{Bayesian Networks}

% old comment
% KM: I Suggest expanding this (the paragraph below) - split into measurement, methods, etc - or leave this part out and absorb refs into the lit review. G, K and F seem quite relevant, but this paragraph doesn't do them justice, so expand or move.
% 
% Review further comments about this paragraph - it's not a good piece of work, but it is essential to consider how impact and sustainability are or could be measured and interpreted. Perhaps go deeper like you did in your confirmation.
% THIS IS WHERE KERRIES RESEARCGH GOOOEOEEESSSS
...
% . However, great complexity is introduced when a node is informed by many parents, which can also have many parents themselves. The change in a single parent node affects all children of the network, often resulting in intractable systems, especially when nodes may have numerous classes 
...
% Specifically, the question of water resources was central to the model and highly interrelated \cite{carmonaUseParticipatoryObjectOriented2011}.
...
% Results: Use the agreed-upon definitions to realise factors and their impact fully.
% 
...
The relationships between the facets in Table~\ref{tab:definitions}, as agreed by the industry panel, are depicted in the BN model in Fig~\ref{fig:generalBN}. The BN highlights which factors others influence in the form of an acyclic-directed graph. Notably, the key factors influencing environmental impact centre around agrochemical sprays, soil management, pest/disease and water. It also illustrates the complex nature of variables such as water and their interrelatedness.
    ...
    % What are theWaterary factors that significantly impact sustainability in the Australian winegrowing industry?
    %     Purpose: Identify the most influential variables directly affecting sustainability and those perceived to directly impact sustainability.
    %     Results: Review the most influential factors and their relation. Compare this to the data in SWA.
    ...
    %     Results: Use the generic BN to illustrate the agreed-upon relationships between discussed factors.
% 


% The BN provided a comprehensive overview of how factors defined by the industry panel would interact. This shows not only direct relationships but also how changes in one factor can propagate through the network and affect others. This understanding is crucial for developing targeted interventions and policies to improve sustainability in the wine-growing industry. This network illustrates the complexity of interactions that determine sustainability in Australian winegrowing. Furthermore, the extent of relationships helps to show the importance of variables such as water intrinsically, which is prolific in how it is interconnected with water vineyard aspects.
...
%     Purpose: Explore and understand both the literature and industry perspectives on this.
%     Results in Literature review. Industry list of factors. Mind Map? Leximancer network.
%     - Use the same words as the definitions in the search to show the parallels and differences.
% Please add the following required packages to your document preamble:
\begin{table}[ht]
    \begin{tabular}{@{}lrr@{}}
    \toprule
    ...
    disease & 646 & 49 \\
    change & 569 & 43 \\
    %TODO: One of these should be something other than change!!! They both can't be!!
    grapevine & 565 & 43 \\
    viticulture & 565 & 43 \\
    soil & 450 & 34 \\
    plant & 425 & 32 \\
